...
```
public/
  index.html          # the entire app shell (≤ 110 KB; enforced by Playwright)
  maplibre-preview.js # opt-in vector renderer
...
{
  "version": 5,
  "regionLabel": "Duluth",       // legacy; the UI always labels the region "Metro Atlanta"
  "ratingPriors": { "C": 4.32, "m": 238, "population": 1705, "source": "…" }, // generated by scripts/rating_priors.py; never hand-edit
...
| **Mobile: map uses `clamp(130px, 22vh, 185px)`**, 125 px at ≤ 600 px tall and 120 px at ≤ 500 px; panel takes the rest, footer hidden. | Target 5 fully visible cards at 375×750 and 3 at 320×568. Location rows are omitted. | §4 |
...
`python3 scripts/rating_priors.py` regenerates `shops.json.ratingPriors` from the combined, deduplicated regional population. Rating refresh, discovery writes and curation writes invoke it. Keep the generated priors with dataset changes so partial and complete scores agree.
